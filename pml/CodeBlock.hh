...
public:
  ...
  inline void Print(ostream&os=cout) {
    ...
    if ( _rst ) {
      os << "always_ff @(posedge " << _clk->name << " or negedge " << _rst->name << ")" << endl;
      PUT_SPACE(_step);
      ...
      os << "always_ff @(posedge " << _clk->name << ") begin" << endl;
      PrintFF(os, _step*2);
      os << "end" << endl;
    }
  }
...
class CBlkLegacyFF : public CCodeBlock
{
   ...
   public: 
	 ...
	 os << "always_ff @(posedge " << _clk->name; 
	 if ( _rst ) {
...
//     _ff (NULL), _init (NULL), _body (NULL) {}
  ...
  inline void Print(ostream&os=cout) {
    os << "// Sequential part of FSM " << _loc << endl;
    _ff->Print(os);
    os << endl;
    
    os << "// Combinational part of FSM " << _loc << endl;
    _body->Print(os);
    os << endl;
  }
  ...
  inline void BuildFSM(CSymbolTab* SymbolTabel) {
    ...
    CStmtCASE *fsm_case = new CStmtCASE("unique case", case_exp, case_items, xx_stmt);
